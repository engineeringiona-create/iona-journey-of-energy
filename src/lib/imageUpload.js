...
import { compressMain, isCompressible } from './imageCompression.js';
...
/* Tries Supabase Storage bucket "site_assets" first (see supabase/schema.sql
   ...
   row rather than object storage. Non-SVG files are resized to a max of
   1920px and re-encoded as WebP (see imageCompression.js) before either path,
   so both the storage bytes and the base64 fallback stay small. */
export async function uploadImage(file, { pageId, key }) {
  ...
  const uploadFile = isCompressible(file) ? await compressMain(file).catch(() => file) : file;
  ...
  if (supabase) {
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from('site_assets')
        .upload(path, uploadFile, { cacheControl: '3600', upsert: false, contentType: uploadFile.type || undefined });
